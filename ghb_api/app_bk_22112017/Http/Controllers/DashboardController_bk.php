...
class DashboardController extends Controller
{
	...
	{
		$items = DB::select("
			select distinct a.org_code, a.org_name
			from org a,
			(
	...
	public function kpi_overall_pie(Request $request)
	{
		...
		foreach ($emp_result as $e) {
			...
			$perspective = DB::select("
				select b.perspective_id, c.perspective_abbr, c.perspective_name, c.color_code, sum(a.weight_percent) perspective_weight, sum(a.weigh_score) total_score
				from appraisal_item_result a
	...
	{	
		$qinput = [];
		$query = "
			select c.org_id, if(c.appraisal_type_id=1,e.emp_name,d.org_name) name, a.item_id, b.item_name, ifnull(a.target_value,0) target_value, ifnull(a.actual_value,0) actual_value, a.weight_percent, a.etl_dttm,
			round(if(ifnull(a.target_value,0) = 0,0,(ifnull(a.actual_value,0)/a.target_value)*100),2) achievement,
			100 - round(if(ifnull(a.target_value,0) = 0,0,(ifnull(a.actual_value,0)/a.target_value)*100),2) urgency,
			(
			select rt.color_code
			from result_threshold rt
		...
		foreach ($items as $i) {
			...
			if (!isset($groups[$key])) {
				$groups[$key] = array(
					'items' => array('color' => $key, 'bubbleHoverColor' => $key, 'data' => array(['x' => $i->urgency, 'y' => $i->weight_percent, 'z' => $i->achievement, 'name' => $i->item_name, 'item_id' => $i->item_id,
					"tooltext" => "<div id='nameDiv'>" .$i->item_name. "</div>{br}Urgency : <b>" . $i->urgency . "</b>{br}Impact : <b>" . $i->weight_percent ."</b>{br}Achievement : <b>" . $i->achievement . "%</b>{br}Last Updated: <b>" . $i->etl_dttm . "<b>"]))
				);
			} else {
	...
	public function kpi_overall(Request $request)
	{
		...
		if ($request->appraisal_type_id == 2) {
			...
			$qinput = [];
			$query = "
				select c.org_id, a.level_id, e.emp_name name, a.item_id, b.item_name, ifnull(a.target_value,0) target_value, ifnull(a.actual_value,0) actual_value, a.etl_dttm, 
			...
			$qinput = [];
			$query = "
				select c.org_id, a.level_id, d.org_abbr name, a.item_id, b.item_name, ifnull(a.target_value,0) target_value, ifnull(a.actual_value,0) actual_value, etl_dttm, 
				round(if(ifnull(a.target_value,0) = 0,0,ifnull(a.actual_value,0) / a.target_value) * 100,2) pct,
				(
				select rt.color_code
				from result_threshold rt
		...
		foreach ($items as $i) {
			...
			if (!isset($groups[$key])) {
				$groups[$key] = array(
					'items' => array('color' => $key, 'bubbleHoverColor' => $key,'data' => array(['x' => $i->target_value, 'y' => $i->actual_value, 'z' => $i->pct, 'name' => $i->name, 'org_id' => $i->org_id, 'level_id' => $i->level_id,
					"tooltext" => "<div id='nameDiv'>" .$i->name. "</div>{br}Target : <b>" . $i->target_value . "</b>{br}Actual : <b>" . $i->actual_value ."</b>{br}Achievement : <b>" . $i->pct . "%</b>{br}Last Updated: <b>" . $i->etl_dttm . "<b>"]))
				);
			} else {
	...
	public function gantt(Request $request)
	{
		...
		foreach ($weekly as $w) {
			$week_list[] = ['start' => $w->format("Y-m-d"), 'end' => $w->modify("+6 day")->format("Y-m-d"), 'label' => "Week ".$week_counter];
			$week_counter++;
		}		
	...
	public function performance_trend(Request $request) 
	{
		...
		if ($frequency->frequency_month_value > 1) {
			...
			if ($request->appraisal_type_id == 2) {
				$emp = Employee::where('emp_id',$request->emp_id)->first();
				$org_list = DB::select("
					SELECT a.org_id, a.emp_id, g.emp_name org_name, e.item_name, f.perspective_name, u.uom_name, c.result_threshold_group_id, b.item_result_id, e.is_show_variance, max(a.etl_dttm) etl_dttm
				...
				$org_list = DB::select("
					SELECT a.org_id, a.emp_id, d.org_name, e.item_name, f.perspective_name, u.uom_name, c.result_threshold_group_id, b.item_result_id, e.is_show_variance, max(a.etl_dttm) etl_dttm
					FROM monthly_appraisal_item_result a
					left outer join appraisal_item_result b
					on a.item_id = b.item_id
					and a.emp_result_id = b.emp_result_id
			...
			foreach ($org_list as $o) {
				...
				if ($request->appraisal_type_id == 2) {
					$query = "
						SELECT a.org_id, e.emp_name org_name, a.appraisal_month_name, a.appraisal_month_no, a.target_value monthly_target, b.target_value yearly_target, b.forecast_value, b.actual_value, a.actual_value sum_actual_value
						#(
					...
					$query = "
						SELECT a.org_id, d.org_name, a.appraisal_month_name, a.appraisal_month_no, a.target_value monthly_target, b.target_value yearly_target, b.forecast_value, b.actual_value, a.actual_value sum_actual_value
						#(
						# select sum(actual_value)
						# from monthly_appraisal_item_result
						# where period_id = a.period_id
				...
				$o->bar_chart = [
					...
					'trendlines' => [
						[
							"line" => [
								[
									"startvalue" => $items[0]->yearly_target,
									"color" => "#1aaf5d",
									"valueOnRight" => "1",
									"displayvalue" => "Target"				
								],
			...
			if ($request->appraisal_type_id == 2) {
				...
				$period = AppraisalPeriod::find($request->period_id);
				$org_list = DB::select("
					select distinct e.org_id, d.emp_id, i.emp_name org_name, d.item_result_id, f.result_threshold_group_id, g.item_name, h.perspective_name,max(a.etl_dttm) etl_dttm,u.uom_name
					from monthly_appraisal_item_result a
				...
				$period = AppraisalPeriod::find($request->period_id);
				$org_list = DB::select("
					select distinct e.org_id, d.emp_id, e.org_name, d.item_result_id, f.result_threshold_group_id, g.item_name, h.perspective_name,max(a.etl_dttm) etl_dttm,u.uom_name
					from monthly_appraisal_item_result a
					left outer join appraisal_period b
					on a.period_id = b.period_id
					left outer join appraisal_frequency c
					...
					if (!empty($action_plans)) {
						$action_item = [
							[
							   ...
							   "fillColor" => "#7FC31C",
							   "link" => "javascript:void(0)",
							   "x" => '$dataset.0.set.'.$action_order.'.x-25',
							   "y" => '$dataset.0.set.'.$action_order.'.starty-15',
							   "tox" => '$dataset.0.set.'.$action_order.'.x+25',
							   "toy" => '$dataset.0.set.'.$action_order.'.starty-40'
							],
							[
							   ...
							   "y" => '$dataset.0.set.'.$action_order.'.starty-16',
							   "radius" => "11",
							],		
							[
							   ...
							   "text" => "SIP",
							   "x" => '$dataset.0.set.'.$action_order.'.x-',
							   "y" => '$dataset.0.set.'.$action_order.'.starty - 27'
							]
						];
	...
	public function branch_performance(Request $request)
	{
		...
		if (empty($request->region_code)) {
			$vector = DB::select("
				...
				where y.is_active = 1
				and avg(if(a.target_value=0,0,(a.actual_value/a.target_value * 100))) between x.begin_threshold and x.end_threshold
				) color_code
				from appraisal_item_result a
				left outer join emp_result b
				on a.emp_result_id = b.emp_result_id
			...
			$org_input = array();
			$org_query = "
				select distinct a.org_id, c.org_name, c.longitude, c.latitude, b.result_threshold_group_id, b.result_score pct
				from appraisal_item_result a
				left outer join emp_result b
				on a.emp_result_id = b.emp_result_id
				left outer join org c
			...
			foreach ($org_list as $o) {
				$qinput = array();
				$query = "
					SELECT air.item_result_id, p.perspective_id, p.perspective_name, air.item_id, air.item_name, u.uom_name, air.org_id, org.org_code, o.org_name, er.result_threshold_group_id,
						air.target_value, air.forecast_value, ifnull(air.actual_value, 0) actual_value,
						ifnull(if(air.target_value = 0, 0, (air.actual_value/air.target_value)*100), 0) percent_target,
						ifnull(if(air.forecast_value = 0, 0, (air.actual_value/air.forecast_value)*100), 0) percent_forecast
					FROM appraisal_item_result air
				...
				$o->color_code = $branch_color[0]->color_code;
		...
		return response()->json(['vector_map' => $vector, 'google_map' => $org_list]);
	}

	public function branch_details(Request $request)
	{
		$org_list = DB::select("
			select distinct a.org_id, c.org_name, c.org_code, b.result_threshold_group_id, b.result_score pct
			from appraisal_item_result a
			left outer join emp_result b
			on a.emp_result_id = b.emp_result_id
			left outer join org c
